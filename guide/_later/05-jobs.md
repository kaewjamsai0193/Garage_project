...
### ปุ่มซื้อด่วน — ของไม่พอ กดไปรับของแล้วกลับมาอนุมัติต่อ

```
หน้าใบงาน (ยังไม่อนุมัติ): แถวอะไหล่ที่จำนวนเกินของในคลัง → ตัวแดง "ขาด 2 ขวด" + ปุ่ม ซื้อด่วน
  → /goods-receipts/new?product=12&qty=2&back=/jobs/7
  → ฟอร์มรับของของเฟส 4 โหมดซื้อด่วน ใส่สินค้าและจำนวนที่ขาดไว้ให้ กรอกแค่ชื่อร้านกับยอดที่จ่าย
  → บันทึก = Lot ใหม่ → กลับ /jobs/7 → ตัวแดงหาย → กดอนุมัติต่อ
```

ไม่มีตารางหรือ endpoint ใหม่ ใช้ `POST /api/goods-receipts` ตัวเดิม (ช่างบันทึกได้อยู่แล้ว)

**backend** — แถวอะไหล่ในรายละเอียดใบงานแนบของคงเหลือไปด้วย หน้าจอจะได้รู้ว่าขาดเท่าไร

```python
# jobs/schemas.py
class JobItemOut(Out):
    ...
    qty_on_hand: Decimal  # ของคงเหลือตอนนี้ ไว้เทียบว่าขาดไหม


# jobs/service.py:job_detail — ตอนประกอบแต่ละแถว
"qty_on_hand": item.product.qty_on_hand,
```

- `qty_on_hand` คือ `column_property` ของเฟส 3 · โหลดใบงานด้วย `selectinload(JobOrder.items).selectinload(JobItem.product)` ไม่งั้นยิง query ทีละแถว

**หน้าใบงาน** — ในแถวอะไหล่ ใต้ชื่อสินค้า

```jsx
// ยังไม่อนุมัติและของไม่พอ: บอกว่าขาดเท่าไร + ปุ่มไปฟอร์มรับของ พร้อมบอกทางกลับมาหน้านี้
const missing = Number(it.qty) - Number(it.qty_on_hand);
{!job.approved && missing > 0 && (
  <p className="flex items-center gap-2 text-sm font-semibold text-danger">
    ขาด {formatQty(missing)} {it.unit}
    <Link className="btn btn-ghost" to={`/goods-receipts/new?product=${it.product_id}&qty=${missing}&back=/jobs/${job.id}`}>
      ซื้อด่วน
    </Link>
  </p>
)}
...
- โชว์เฉพาะก่อนอนุมัติ อนุมัติแล้วของถูกตัดไปแล้ว
- ข้อความในป๊อปอัพเพิ่มอะไหล่เปลี่ยนเป็น "ไม่พอ ใส่ไว้ก่อนได้ แล้วกดซื้อด่วนที่แถวนี้"

**`GoodsReceiptFormPage.jsx` (ไฟล์เฟส 4)** — รับค่าจาก URL แล้วพากลับหน้าใบงาน

```jsx
// มาจากหน้าใบงาน: ?product=&qty=&back=/jobs/7 · back รับแค่ /jobs/ กันลิงก์พาไปหน้าอื่น
const back = params.get("back")?.startsWith("/jobs/") ? params.get("back") : null;
const done = back ? () => navigate(back) : close;
const fromJob = useQuery({ queryKey: ["products", params.get("product")], enabled: !!params.get("product") });

// ใส่สินค้าที่ขาดไว้ให้ ใช้ replace เพราะรันซ้ำกี่รอบก็ได้ผลเท่าเดิม (append จะได้สองแถว)
useEffect(() => {
  const p = fromJob.data;
  if (!p) return;
  replace([{ ...EMPTY_ITEM, product_id: p.id, label: `${p.code} — ${p.name} (${p.unit})`, qty: plainNumber(params.get("qty")) }]);
}, [fromJob.data, params, replace]);
```

แก้อีกสองจุดในไฟล์เดิม:

```jsx
// onSuccess: กลับหน้าใบงานถ้ามาจากที่นั่น
if (back) queryClient.invalidateQueries({ queryKey: ["jobs"] });
navigate(back ?? `/goods-receipts/${gr.id}`);

// ปิดป๊อปอัพก็กลับหน้าใบงานเหมือนกัน
<Modal title="บันทึกรับของ" onClose={done} ...>
```

- โหมดเริ่มเป็น "ซื้อด่วน" อยู่แล้ว เพราะไม่มี `?po=`
- เพิ่มสินค้าอื่นในใบเดียวกันได้ตามปกติ (ซื้อหลายอย่างจากร้านเดียว)

**ข้อจำกัดที่รู้อยู่แล้ว**

| เรื่อง | เพราะ |
|---|---|
| ใบรับของไม่ได้ผูกกับใบงาน ย้อนดูไม่รู้ว่าซื้อมาให้งานไหน | ไม่เพิ่มคอลัมน์ ถ้าต้องการค่อยเพิ่ม `goods_receipts.job_id` |
| ไม่มีการจองของ อีกใบงานอนุมัติก่อนอาจตัดของที่เพิ่งซื้อไป | อู่สาขาเดียวโอกาสน้อย ตรวจจริงยังอยู่ที่ `approve` |
| อะไหล่ที่ยังไม่มีในระบบ ต้องให้พนักงานเพิ่มสินค้าก่อน | ช่างสร้างสินค้าไม่ได้ (ยังไม่ได้ตัดสินใจเรื่องนี้) |
...
- แถวที่ของไม่พอขึ้น **"ขาด 1"** + ปุ่มซื้อด่วน → กด → ฟอร์มรับของมีสินค้ากับจำนวนให้แล้ว → กรอกร้าน ยอดจ่าย → บันทึก → **กลับมาหน้าใบงาน ตัวแดงหาย** → อนุมัติได้ · กดปิดฟอร์มเฉย ๆ ก็กลับหน้าใบงาน
